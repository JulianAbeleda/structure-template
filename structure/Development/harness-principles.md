...
Use this file when a project has benchmark, evaluator, experiment, or search harnesses whose output can influence
engineering decisions.

The goal is not to require one benchmark framework. The goal is to make claims reproducible enough that a future reader
can tell what was measured, why it mattered, and whether it should change product behavior.
...
compile or warmup handling, dispatch path, or promotion policy, it can change the result.
...
|---|---|---|
| whole-system or end-to-end gate | final product-facing performance, quality, or behavior claim | yes |
| isolated local A/B gate | diagnostic comparison against the current winner | no |
| profiler or tracing output | attribution and root-cause analysis | no |
| debug logs or print timing | debugging only | no |
| synthetic microbench | diagnostic only unless the shipped path is the same synthetic path | no |

Promotion is reported by a harness, not silently applied by it. Default, policy, and rollout changes should remain an
...
2. candidate id and primitive, feature, or behavior class;
...
8. repeats, median, spread, and reproducibility or noise band;
...
For approximate, stochastic, or reordered behavior, state:

- the tolerance;
- the reference output or behavior;
- why the tolerance is valid;
...
Do not invent one-off verdict strings in child harnesses if a central evaluator, schema, or ledger consumes them.
